...
function fcShowDetail(id) {
    ...
    if (fcIsDesktop()) {
        // PC: overlay modal
        fcCloseDetail(); // close any existing
        const overlay = document.createElement('div');
        overlay.className = 'fc-detail-overlay';
        overlay.id = 'fcOverlay';
        overlay.innerHTML = `
            <div class="fc-detail-modal">
                <div class="fc-modal-header">
                    <div class="fc-modal-title">${fcEsc(ev.title)}</div>
                    <button class="fc-modal-close" onclick="fcCloseDetail()" title="CLOSE">&times;</button>
                </div>
                <div class="fc-modal-body">${bodyHtml}</div>
            </div>
        `;
        ...
        // Mobile: inline detail below current view
        fcCloseDetail();
...
function fcCloseDetail() {
    ...
    // Remove inline detail if exists
    const inline = document.getElementById('fcInlineDetail');
    if (inline) inline.remove();
    // Also hide legacy right-column panel
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape' && fcSelectedId !== null) {
        fcCloseDetail();
    }
});
